...
public class MrzParser {
     ...
     */
    public MrzDate parseDate(MrzRange range) throws RuntimeException{
        if (range.length() != 6) {
        ...
        try {
            ...
            year = -1;
            throw  new RuntimeException("Failed to parse MRZ date year " + rawValue(range) + ": " + ex + mrz + r);
        }
        if (year < 0 || year > 99) {
            throw  new RuntimeException("Invalid year value " + year + ": must be 0..99");
        }
        ...
        try {
            ...
            month = -1;
            throw  new RuntimeException("Failed to parse MRZ date month " + rawValue(range) + ": " + ex+mrz+ r);
        }
        if (month < 1 || month > 12) {
            throw  new RuntimeException("Invalid month value " + month + ": must be 1..12");
        }
        ...
        if (day < 1 || day > 31) {
            throw  new RuntimeException("Invalid day value " + day + ": must be 1..31");
        }
