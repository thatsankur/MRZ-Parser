...
public class MrzDate implements Serializable, Comparable<MrzDate> {
    ...
    public MrzDate(int year, int month, int day, String raw) throws RuntimeException {
        this.year = year;
        this.month = month;
        this.day = day;
        isValidDate = check();
    ...
    private boolean check() throws RuntimeException {
        if (year < 0 || year > 99) {
            throw  new RuntimeException("Parameter year: invalid value " + year + ": must be 0..99");

        }
        if (month < 1 || month > 12) {
            throw  new RuntimeException("Parameter month: invalid value " + month + ": must be 1..12");

        }
        if (day < 1 || day > 31) {
            throw  new RuntimeException("Parameter day: invalid value " + day + ": must be 1..31");
